...
import type { ProgressState } from "./SeoTaskCard";
...
type Market = "sa" | "id" | "my" | "ae" | "uk";

// Mock 结果数据（mini-MVP 阶段示意，D8 接真 N8N 后替换）
const MOCK_ROWS = [
  { keyword: "hijab", volume: 110000, kd: 42, cpc: 0.85, comp: "中" },
  { keyword: "abaya", volume: 74000, kd: 38, cpc: 0.62, comp: "中" },
  { keyword: "ramadan dates", volume: 18100, kd: 29, cpc: 0.41, comp: "低" },
  { keyword: "modest fashion", volume: 9900, kd: 47, cpc: 1.12, comp: "高" },
  { keyword: "prayer mat", volume: 6600, kd: 35, cpc: 0.78, comp: "中" },
];
...
export function W01Workspace() {
      ...
      try {
        ...
        setProgress((p) => {
          ...
          if (evt.node_status === "failed") {
            next.errorMessage = (payload && payload.error) || "节点失败";
          }
          return next;
        });
  ...
  if (status === "idle") {
    ...
    statusText = `处理中… step ${progress.seq ?? 0}/${progress.totalEstimated ?? 6}`;
    statusTextCls = "text-emerald-700";
  } else if (status === "succeeded") {
    const parts = [`已完成 · 写入 ${progress.rowsNew ?? 0} 行`];
    if (progress.rowsCached != null)
      parts.push(`命中缓存 ${progress.rowsCached} 行`);
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      ...
      <div className="px-5 py-3 border-b border-gray-200 bg-white shrink-0">
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <select
            ...
            className="h-7 w-36 rounded border border-gray-300 px-2 text-xs text-gray-900 outline-none focus:border-emerald-400"
          >
            {MARKETS.map((m) => (
      ...
      <div className="flex-1 overflow-auto bg-white">
        ...
        {status === "succeeded" && <ResultTable rows={MOCK_ROWS} />}
...
// 结果表（mini-MVP 用 mock 5 行示意；D8 切真 N8N 后替换为真数据）
function ResultTable({ rows }: { rows: typeof MOCK_ROWS }) {
  return (
    <div>
      {/* 结果表头部：左侧标题 + 右侧 "示意数据" amber chip */}
      <div className="px-5 py-2 flex items-center justify-between border-b border-gray-200">
        <span className="text-sm font-medium text-gray-700">返回结果</span>
        <span className="rounded border border-amber-500 bg-amber-50 px-2 py-0.5 text-xs text-amber-700">
          示意数据 · D8 切真后替换
        </span>
      </div>
      ...
      <table className="w-full text-xs">
        <thead className="sticky top-0 bg-gray-50 text-gray-500">
          <tr className="border-b border-gray-200">
            ...
            <th className="px-5 py-2 text-left font-medium">竞争度</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            ...
            >
              ...
              <td className="px-5 py-2 text-right text-gray-700">{r.kd}</td>
              <td className="px-5 py-2 text-right text-gray-700">
                {r.cpc.toFixed(2)}
              </td>
              <td className="px-5 py-2 text-gray-700">{r.comp}</td>
            </tr>
          ))}
        </tbody>
      </table>
