...
import type { ProgressState, W05ResultRow } from "./SeoTaskCard";
...
const ENDPOINT_KEY = "W05";
const DATA_SOURCE = "semrush_kmt_staging（kmt_mode='related'）";
...
export function W05Workspace() {
  ...
  const [history, setHistory] = useState<HistoryEntry<W05ResultRow>[]>([]);
  ...
  useEffect(() => {
    let alive = true;
    loadHistory<W05ResultRow>(ENDPOINT_KEY).then((h) => {
  ...
  useEffect(() => {
    if (
      ...
      appendHistory<W05ResultRow>(ENDPOINT_KEY, {
  ...
  function startStream() {
    ...
    es.addEventListener("rows", (ev) => {
      try {
        const data = JSON.parse((ev as MessageEvent).data) as { rows: W05ResultRow[] };
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      ...
      <div className="flex-1 overflow-auto bg-[rgba(6,16,11,.6)]">
        {historyMode ? (
          <HistoryView<W05ResultRow>
            entries={history}
            renderTable={(rs) => <RelatedResultTable rows={rs} />}
            extraRightControls={
            <button
              type="button"
              onClick={() => setToolbarCollapsed((v) => !v)}
            ...
            onRemove={(id) =>
              removeHistoryEntry<W05ResultRow>(ENDPOINT_KEY, id)
                .then(setHistory)
                .catch(() => {})
            }
            onClear={() => {
...
const W05_GETTERS: Record<string, Getter<W05ResultRow>> = {
  market: (r) => r.market,
  keyword: (r) => r.keyword,
...
function RelatedResultTable({ rows }: { rows: W05ResultRow[] }) {
    ...
    <div>
      <div className="px-5 py-2 flex items-center justify-between border-b border-manor-brass/25 bg-manor-bg3/40">
        ...
        <span className="text-[10px] tracking-[0.22em] text-manor-brassDim font-sc" style={{ fontFamily: "var(--font-sc), 'Cormorant SC', serif" }}>
          数据源：semrush_kmt_staging（kmt_mode=&apos;related&apos;）
        </span>
      </div>
        ...
        <tbody>
          {sorted.map((r) => (
            ...
            >
              ...
              <td className="px-5 py-2 text-right text-manor-ink">
                {r.relevance_rate != null ? r.relevance_rate.toFixed(2) : "—"}
              </td>
