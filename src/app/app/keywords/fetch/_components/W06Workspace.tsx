...
const ENDPOINT_KEY = "W06";
const DATA_SOURCE = "semrush_kmt_staging（kmt_mode='fullsearch'）";
...
export function W06Workspace() {
  ...
  const [rows, setRows] = useState<W06ResultRow[]>([]);
  ...
  const [history, setHistory] = useState<HistoryEntry<W06ResultRow>[]>([]);
  ...
  useEffect(() => {
    setHistory(loadHistory<W06ResultRow>(ENDPOINT_KEY));
  }, []);
  ...
  useEffect(() => {
    if (
      ...
      const next = appendHistory<W06ResultRow>(ENDPOINT_KEY, {
  ...
  function startStream() {
    ...
    es.addEventListener("rows", (ev) => {
      try {
        const data = JSON.parse((ev as MessageEvent).data) as { rows: W06ResultRow[] };
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      ...
      <div className="flex-1 overflow-auto bg-white">
        {historyMode ? (
          <HistoryView<W06ResultRow>
            entries={history}
            renderTable={(rs) => <FullsearchResultTable rows={rs} />}
            onClose={() => setHistoryMode(false)}
            onRemove={(id) =>
              setHistory(removeHistoryEntry<W06ResultRow>(ENDPOINT_KEY, id))
            }
            onClear={() => {
        ...
        {status === "succeeded" && (
          rows.length > 0 ? (
            <FullsearchResultTable rows={rows} />
          ) : (
    ...
    <div>
      <div className="px-5 py-2 flex items-center justify-between border-b border-gray-200">
        <span className="text-sm font-medium text-gray-700">
          全搜索拓词 · 共 {rows.length} 行
        </span>
        <span className="text-[11px] text-gray-400">
          数据源：semrush_kmt_staging（kmt_mode=&apos;fullsearch&apos;）
        </span>
      </div>
