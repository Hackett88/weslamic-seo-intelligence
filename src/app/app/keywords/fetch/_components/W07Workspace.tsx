...
import type { ProgressState, W07ResultRow } from "./SeoTaskCard";
import { HistoryView } from "./HistoryView";
import { SortableTh, useTableSort, type Getter } from "./SortableTable";
import { formatIntent } from "../../_components/_utils";
import {
  appendHistory,
  clearHistory,
  loadHistory,
...
export function W07Workspace() {
  ...
  const [rows, setRows] = useState<W07ResultRow[]>([]);
  ...
  const [history, setHistory] = useState<HistoryEntry<W07ResultRow>[]>([]);
  ...
  function startStream() {
    ...
    es.addEventListener("rows", (ev) => {
      try {
        const data = JSON.parse((ev as MessageEvent).data) as { rows: W07ResultRow[] };
  ...
  async function handleSubmit() {
    if (noKeyword || !limitInRange) return;
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      {/* 顶部工具栏：keyword + display_limit（W07 不接受 market 入参，跨库返回所有数据库该词搜量） */}
      {!toolbarCollapsed && (
...
const W07_GETTERS: Record<string, Getter<W07ResultRow>> = {
  market: (r) => r.market,
  keyword: (r) => r.keyword,
...
function CrossMarketTable({ rows }: { rows: W07ResultRow[] }) {
  // 默认按月搜量降序，让最大市场置顶
    ...
    <div>
      <div className="px-5 py-2 flex items-center justify-between border-b border-manor-brass/25 bg-manor-bg3/40">
        <span className="text-sm font-semibold text-manor-brassHi tracking-[0.06em]" style={{ fontFamily: "var(--font-serif), 'EB Garamond', serif" }}>
          跨市场搜量 · 共 {rows.length} 行
        </span>
        <span className="text-[10px] tracking-[0.22em] text-manor-brassDim font-sc" style={{ fontFamily: "var(--font-sc), 'Cormorant SC', serif" }}>
          数据源：semrush_kmt_staging（kmt_mode=&apos;all&apos;）
        </span>
      </div>
